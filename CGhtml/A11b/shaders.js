...
void main() {
	fs_pos = in_pos;
	fs_norm = in_norm;
	fs_Ldir = normalize(LAPos - in_pos);
	fs_LAlightColor = LAlightColor;
	gl_Position = pMatrix * vec4(in_pos, 1.0);
}
`;
...
void main() {
	vec3 norm = normalize(fs_norm);
	// diffuse
	vec4 diffuse = diffuseColor * max(dot(norm, fs_Ldir), 0.0);
	// specular
	vec3 eyeDir = normalize(eyePos - fs_pos);
	...
	vec4 ambient = ambientLightColor * ambientMatColor;
	// final
	vec4 finalColor = clamp((diffuse + specular) * fs_LAlightColor + ambient + emitColor, 0.0, 1.0);
...
void main() {
	...
	fs_LAlightColor = LAlightColor;
	// diffuse
	diffuse = diffuseColor * max(dot(normalize(fs_norm), fs_Ldir), 0.0);
